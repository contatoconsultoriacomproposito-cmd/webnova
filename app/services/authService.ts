...
    email: email,
    // CORREÇÃO: Se o perfil não tiver role, OU se o role for
    // o valor padrão 'INSTITUCIONAL', Mapeamos o frontend para NO_PLAN.
    plan: (!profile.role || profile.role === PlanType.INSTITUTIONAL) 
        ? PlanType.NO_PLAN // AGORA CORRETO: Atribui NO_PLAN quando não há plano pago
...
    // Fallback para usuário básico com NO_PLAN (ESTE JÁ ESTAVA CERTO!)
    return {
        id: session.user.id,
